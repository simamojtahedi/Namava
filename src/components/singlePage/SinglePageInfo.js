...
const SinglePageInfo = ({data, recommend}) => {
    function mediaDetails (caption, items, maxLength, keyType ) {
        ...
        for (let i= 0; i < maxLength && i < items.length; i++) {
            content.push(<span to='/' key={items[i].categoryId || items[i].castId || items[i].categoryId || items[i].languageId}>{items[i][keyType]}</span>)
            content.push(<span to='/' key={`seprator${items[i].categoryId || items[i].castId || items[i].categoryId || items[i].languageId}`} className='seprator'> - </span>)
        }
        content.pop()
        return (
        ...
        <div className='single-page-maininfo mt-4'>
            ...
            <div className='casts mt-5'>
                ...
                >
                    {data?.casts.length > 0 && 
                        data.casts.filter(item => item.castRole !== 'Actor').map(item => (
                            <SwiperSlide key={`${item.castRole}-${item.castId}`} >
                                <Link to='/'>
                                    <LazyLoad className="cast-placeholder" >
                                        <img src={item.castImageUrl ? imageUrl(item.castImageUrl) : 'data:image/svg+xml;base64,'} />
                                    </LazyLoad>
